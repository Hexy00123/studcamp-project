...
def ml_profitability_pipeline():
    ...
    def select_and_deploy_best_model(model_metrics_list: list[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        model_name = best_model['model_s3_path'].split('/')[0].split('_')[-1] if '_' in best_model['model_s3_path'] else "Unknown"
        
        print(f"Best model: {model_name} - Test R2: {best_model['test_metrics']['r2']:.4f}, Train R2: {best_model['train_metrics']['r2']:.4f}")
        ...
        copy_source = {'Bucket': BUCKET_NAME, 'Key': best_model['model_s3_path']}
        s3_client.client.copy_object(CopySource=copy_source, Bucket=BUCKET_NAME, Key='models/best/model.pkl')
        
        model_timestamp = best_model['model_s3_path'].split('/')[1]
        try:
            copy_source = {'Bucket': BUCKET_NAME, 'Key': f"models/{model_timestamp}/results.json"}
            s3_client.client.copy_object(CopySource=copy_source, Bucket=BUCKET_NAME, Key='models/best/results.json')
        except Exception:
            pass
    ...
    rf_results = train_model_with_grid_search(preprocessing_results, model=RandomForestRegressor, params={'model__n_estimators': [50, 100], 'model__max_depth': [20, 35], 'model__n_jobs': [-1]})
    rf_metrics = push_final_metrics_to_xcom(rf_results)

    gb_results = train_model_with_grid_search(preprocessing_results, model=HistGradientBoostingRegressor, params={'model__max_depth': [3, 4, 6]})
    gb_metrics = push_final_metrics_to_xcom(gb_results) 

    preprocessing_results >> rf_results >> rf_metrics
    preprocessing_results >> gb_results >> gb_metrics
    select_and_deploy_best_model([rf_metrics, gb_metrics])
